...
function calculateOffer(payload) {
  ...
  // --- vervoer (briefpunt 11, ronde 47; klantveld verwijderd in ronde 48) ---
  ...
  // Dat is sinds ronde 47 volledig verwijderd: een onbekende afstand wordt
  // NOOIT meer behandeld alsof er daadwerkelijk is gereden. Ronde 48 heeft
  // het klantzichtbare invoerveld voor de retourafstand volledig uit de
  // offertewizard verwijderd (reisafstand bepalen is een interne
  // verantwoordelijkheid van Brabantschoon, niet iets wat aan een
  // potentiële klant wordt gevraagd) — calc.retourKm wordt vanuit de
  // wizard dus niet meer aangeleverd en is in de praktijk altijd
  // undefined. Deze regel blijft bewust bestaan als intern/toekomstig
  // uitbreidingspunt (bijv. handmatige invoer door Brabantschoon zelf,
  // of een latere routeprovider-integratie) zonder dat daarvoor nu al
  // client-input bestaat: parseRetourKm(undefined) geeft altijd null,
  // dus km=null, voertuigkosten=€0 in de voorlopige kostprijs, en
  ...
  // vervoerskosten. Er wordt hier nooit een afstand verzonnen (geen
  // 20-km-fallback, geen postcode-gok, geen Maps/API-integratie). Dit
  // verlaagt bewust NIET de betrouwbaarheid van de schoonmaaktijd zelf
  // (dat is een ander soort onzekerheid) — het is een ontbrekende
  // commerciële kostenpost, geen tijdsinschattingsprobleem.
